...
public class TestConfigBuilder extends BasicTest {
        ...
        public void testAll() {
                ...
                Assertions.assertEquals(2,
                                basicConfig.getProfiles().get("test").getApp().getRevisionHistoryLimit().intValue());
                Assertions.assertEquals(8080, basicConfig.getProfiles().get("test").getApp().getPort().intValue());
                Assertions.assertEquals("*.yaml",
                                basicConfig.getProfiles().get("test").getIgnoreChangeFiles().iterator().next());
                Assertions.assertEquals("127.0.0.1", basicConfig.getProfiles().get("test").getDocker().getHost());

                String projectConfigStr = "namespace: dew-spec\n"
                                + "app:\n"
                ...
                Assertions.assertEquals(5,
                                projectConfig.getProfiles().get("test").getApp().getRevisionHistoryLimit().intValue());
                Assertions.assertEquals(8080, projectConfig.getProfiles().get("test").getApp().getPort().intValue());
                Assertions.assertEquals("*.yaml",
                                projectConfig.getProfiles().get("test").getIgnoreChangeFiles().iterator().next());
                Assertions.assertEquals("127.0.0.2", projectConfig.getProfiles().get("test").getDocker().getHost());
                Assertions.assertEquals("sunisle",
                                projectConfig.getProfiles().get("test").getDocker().getRegistryUserName());
