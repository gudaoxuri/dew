...
public class WebTest {
        ...
        private void testHashCollision() {
                String body = IntStream.range(0, 914748)
                                ...
                                .collect(Collectors.joining(",",
                                                "{\"idCard\":\"110101201709013174\",\"age\":20,\"phone\":\"18651111111\",\"dos\":{",
                                                "}}"));
                ...
                /*
                 * WebExampleController.User user = $.json.toObject(str,
                 * WebExampleController.User.class);
                 * Assertions.assertEquals("110101201709013174", user.getIdCard());
                 */
        }

        private void testResponseFormat() {
                Resp<String> result = Resp.generic(testRestTemplate.getForObject("/test/success?q=TEST", Resp.class),
                                String.class);
                Assertions.assertEquals("successful", result.getBody());
                result = Resp.generic(testRestTemplate.getForObject("/test/badRequest?q=TEST", Resp.class),
                                String.class);
                Assertions.assertEquals("badrequest", result.getMessage());
                ResponseEntity<Resp> resultEntity = testRestTemplate.getForEntity("/test/customError?q=TEST",
                                Resp.class);
        ...
        private void testValidation() {
                ...
                ResponseEntity<Resp> createResult = testRestTemplate.postForEntity("/test/valid-create", userDTO,
                                Resp.class);
                ...
                ResponseEntity<Resp> updateResult = testRestTemplate.postForEntity("/test/valid-update", map,
                                Resp.class);
                ...
                // path
                ResponseEntity<Resp> pathResult = testRestTemplate.getForEntity("/test/valid-method-spring/1",
                                Resp.class);
