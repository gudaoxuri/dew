...
@Configuration
@ConditionalOnClass({RabbitTemplate.class, Scope.class})
...
public class OpenTracingAutoConfiguration {
    ...
    public void init() {
        ...
        RabbitClusterMQ.setSendErrorFun((ex, beforeResult) -> RabbitMqSpanDecorator.onError(ex, ((Scope) beforeResult).span()));
        RabbitClusterMQ.setSendFinishFun(beforeResult -> ((Scope) beforeResult).close());
        ...
        RabbitClusterMQ.setReceiveErrorFun((ex, beforeResult) -> ((Optional<Scope>) beforeResult).ifPresent(scope -> RabbitMqSpanDecorator.onError(ex, scope.span())));
        RabbitClusterMQ.setReceiveFinishFun(beforeResult -> ((Optional<Scope>) beforeResult).ifPresent(Scope::close));
