...
# 添加本地Host映射
-
x.x.x.x todo-uat.dew.idealworld.group
x.x.x.x todo-api-uat.dew.idealworld.group
-
# ======= 前置准备 =======
...
# 打开浏览器，输入 http://todo-uat.dew.idealworld.group，切换到移动模式体验
----
...
. 为这个Kubernetes namespace创建Ingress，绑定服务与域名，内容类似：
  ...
  spec:
    rules:
    - host: todo-api-uat.dew.idealworld.group
      http:
        paths:
        - backend:
            ...
            servicePort: 8080
          path: /
    - host: todo-uat.dew.idealworld.group
      http:
        paths:
        - backend:
...
当执行 ``mvn -P devops deploy -Ddew_devops_profile=uat`` 时实际上做了：
